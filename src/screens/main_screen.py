...
class MainScreen:
    def __init__(self, display, input_handler, pet):
        ...
        # Layout positions for icons
        self.icon_spacing = 5
        total_icons_width = self.num_icons * ICON_SIZE[0] + (self.num_icons - 1) * self.icon_spacing
        icon_start_x = (self.width - total_icons_width) // 2
        
        self.icon_positions = []
        for i in range(self.num_icons):
            x = icon_start_x + i * (ICON_SIZE[0] + self.icon_spacing)
            y = 5  # Top padding
    ...
    def load_icons(self):
        """Load toolbar icons"""
        icon_files = ["Eat.png", "Sleep.png", "Clean.png", "Heal.png", "Conversation.png"]
        icons = []
    ...
    def update(self):
        ...
        if self.input.is_pressed("up") and self.pet.needs_conversation:
            # Go to conversation screen
            return (ScreenType.CONVERSATION, None)
            
        if self.input.is_pressed("right"):
            # Move to next icon
            self.active_icon_index = (self.active_icon_index + 1) % self.num_icons
        ...
        if self.input.is_pressed("press") or self.input.is_pressed("key1"):
            ...
            if self.active_icon_index == 0:  # Eat
                ...
                self.pet.heal()
            elif self.active_icon_index == 4:  # Conversation
    ...
    def draw(self):
        ...
        if self.show_up_arrow and "up" in self.arrows:
            ...
            arrow_y = 5 + ICON_SIZE[1] + 5  # Below icons
