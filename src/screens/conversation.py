...
class ConversationScreen:
    def __init__(self, display, input_handler, pet, ai_handler):
        ...
        # Speech bubble dimensions (from wireframe)
        self.bubble_width = 118
        self.bubble_height = 50
        
        # Calculate positions
        self.pet_pos = (self.width // 2 - PET_SIZE[0] // 2, 
                        self.height - PET_SIZE[1] - 10)  # Bottom center
                        
        self.bubble_pos = (self.width // 2 - self.bubble_width // 2,
                          self.pet_pos[1] - self.bubble_height - 10)  # Above pet
                          
        # Load up arrow
        self.up_arrow = self.load_up_arrow()
    ...
    def load_up_arrow(self):
        """Load up arrow indicator"""
        try:
            arrow_path = f"{INDICATORS_PATH}/up_arrow.png"
            image = pygame.image.load(arrow_path).convert_alpha()
            return pygame.transform.scale(image, ARROW_SIZE)
        except Exception as e:
            print(f"Error loading up arrow: {e}")
